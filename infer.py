...
import numpy as np
from utils import find_pairs_in_dir, compute_mix_scale_from_multispec, stft_torch, istft_torch, make_cond_features_from_multispec, cond_channel_count
from datasets.paired_stft import PairedSTFTDataset
from models.simple_beamformer import SimpleBeamformer
...
def inference_and_save(encoder, pairs, out_dir, args, device):
    ...
    for mix_path, ref_path in pairs:
        ...
        spec_list = []
        for m in range(M):
            mic_wave = torch.from_numpy(mix_wav_m[:, m].astype(np.float32)).unsqueeze(0)
            S = stft_torch(mic_wave, n_fft=args.n_fft, hop_length=args.hop, win_length=args.win_length)[0]
            spec_list.append(S.unsqueeze(0))
        mix_spec_multi = torch.cat(spec_list, dim=0).unsqueeze(0).to(device)
        ...
        try:
            wav_hat = istft_torch(beam_spec_denorm, n_fft=args.n_fft, hop_length=args.hop, win_length=args.win_length, length=mix_wav_m.shape[0])
...
def main():
    ...
    # Attempt to recover M by scanning test_dir
    pairs, _ = find_pairs_in_dir(args.test_dir, recursive=True, verbose=True)
    if not pairs:
